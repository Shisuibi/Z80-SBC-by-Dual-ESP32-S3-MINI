...
　62H　｜　幾何学演算環境光源係数（0.0～1.0を8ビットで設定）
　63H　｜　[IN] 拡散反射係数（1.0－環境光源係数）　[OUT] 幾何学演算平行光源方向（座標XYZWで設定）
...
　68H　｜　[IN] 行列階層　[OUT] 幾何学演算回転X軸（角度は-180度～+180度を8ビットで設定）
　69H　｜　[IN] 行列階層　[OUT] 幾何学演算回転Y軸（角度は-180度～+180度を8ビットで設定）

　6AH　｜　[IN] 行列階層　[OUT] 幾何学演算回転Z軸（角度は-180度～+180度を8ビットで設定）
　6BH　｜　[IN] 行列階層　[OUT] 幾何学演算行列乗算（値は不問）

　6CH　｜　[IN] 行列階層　[OUT] 幾何学演算平行移動（座標XYZWで設定）
　6DH　｜　[IN] 行列階層　[OUT] 幾何学演算拡大縮小（座標XYZWで設定）

　6EH　｜　[IN] 行列階層　[OUT] 幾何学演算表示機器行列（値は不問）
　6FH　｜　[IN] 行列階層　[OUT] 幾何学演算透視変換行列（画角は180度を8ビット、距離は座標XYで設定）

　70H　｜　[IN/OUT] 幾何学演算模型消去（値は不問）
　71H　｜　[IN] 追加済の模型数　[OUT] 幾何学演算模型追加・変更（模型番号）

　72H　｜　[IN] 追加済の頂点数　[OUT] 幾何学演算頂点追加・変更（頂点番号、内容は座標XYZWで設定）
　73H　｜　[IN] 追加済の多角数　[OUT] 幾何学演算多角追加・変更（多角番号、内容は座標XYZWで設定）

　74H　｜　[IN] 最後に処理した模型の可否　[OUT] 幾何学演算模型登録（模型番号、現在の行列で処理）
　75H　｜　[IN] 最後に処理した模型の可否　[OUT] 幾何学演算模型陰影（模型番号、現在の行列で処理）
